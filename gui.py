...
# NOVO: Importe as duas novas funções do banco de dados
from database import registrar_entrada, registrar_saida, exportar_excel, fechar_conexao, buscar_setores, buscar_veiculos_pendentes, buscar_carreta_por_veiculo
import os
import sys

# NOVO: Função para atualizar a lista do Combobox de saída
def atualizar_lista_veiculos_saida():
    veiculos_pendentes = buscar_veiculos_pendentes()
    saida_placa_veiculo['values'] = veiculos_pendentes

# NOVO: Função que puxa a placa da carreta quando o veículo é selecionado
def preencher_carreta_saida(event=None):
    placa_selecionada = saida_placa_veiculo.get()
    if placa_selecionada:
        carreta = buscar_carreta_por_veiculo(placa_selecionada)
        saida_placa_carreta.delete(0, tk.END) # Limpa o campo
        saida_placa_carreta.insert(0, carreta) # Insere a nova placa
...
def registrar_entrada_gui():
    ...
    if placa_veiculo and placa_carreta and motorista and cpf and nota_fiscal and setor and transportadora and empresa:
        ...
        # NOVO: Atualiza a lista de saída para o novo caminhão aparecer lá na hora
        atualizar_lista_veiculos_saida()
    else:
...
def registrar_saida_gui():
    ...
    if placa_veiculo and placa_carreta:
        try:
            ...
            saida_placa_veiculo.set('') # NOVO: Limpa o combobox
            saida_placa_carreta.delete(0, tk.END)
            
            # NOVO: Atualiza a lista para remover o caminhão que acabou de sair
            atualizar_lista_veiculos_saida()
...
icon_path = os.path.join(base_path, "PYTHON.ico")
# Comentei a linha abaixo caso o ícone não esteja na mesma pasta na hora do teste rápido, desfaça se precisar.
...
# Colocando widgets dentro do frame rolável (ENTRADA)
...
# Colocando widgets dentro do frame rolável (SAÍDA)
...
# NOVO: Modificado de tk.Entry para ttk.Combobox
saida_placa_veiculo = ttk.Combobox(scrollable_frame, state="readonly")
saida_placa_veiculo.grid(row=13, column=1, padx=10, pady=10)
# NOVO: Amarrando o evento de selecionar um item à função de preencher a carreta
saida_placa_veiculo.bind("<<ComboboxSelected>>", preencher_carreta_saida)
...
# NOVO: Carregar a lista de veículos pendentes assim que o app abrir pela primeira vez
atualizar_lista_veiculos_saida()
